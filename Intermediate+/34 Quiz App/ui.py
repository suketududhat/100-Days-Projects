...
class QuizInterface:
    def __init__(self, quiz_brain: QuizBrain):
        ...
        self.question_text = self.canvas.create_text(
            150, 125, text="test", fill=THEME_COLOR, font=("Arial", 20, "italic")
        )
        ...
        self.true_button = Button(
            image=true_img, highlightthickness=0, command=self.get_next_question
        )
        self.true_button.grid(row=2, column=0)

        false_img = PhotoImage(file="Intermediate+/34 Quiz App/images/false.png")
        self.false_button = Button(
            image=false_img, highlightthickness=0, command=self.get_next_question
        )
